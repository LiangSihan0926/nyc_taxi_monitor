...
from taxi_monitor.analytics import weekday_weekend_demand
...
def test_consensus_anomalies_runs():
    demand = _toy_demand()
    demand.loc[(demand["zone_id"] == 1) & (demand["pickup_hour"] == demand["pickup_hour"].max()), "trips"] = 100
...
def test_dashboard_builds(tmp_path: Path):
    ...
    fig = build_dashboard(demand, forecast=future, output_path=tmp_path / "dashboard.png")
